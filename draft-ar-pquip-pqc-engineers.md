...
* [BIKE](https://bikesuite.org/): Based on the the hardness of syndrome decoding of QC-MDPC codes. Quasi-Cyclic Moderate Density Parity Check (QC-MDPC) code are a class of error correcting codes that leverages bit flipping technique to efficiently correct the Hamming weight proportional to square root of the code length.
* [HQC](http://pqc-hqc.org/) : Based on the hardness of syndrome decoding of Quasi-cyclic BCH codes. BCH are a class of cyclic error-correcting codes that are constructed using polynomials in the Galois field and provide a great generalization of the Hamming code for multiple bit-error correction. 
